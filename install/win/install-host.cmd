...
setlocal
  ...
  for /f "tokens=2*" %%a in ('HKEY_LOCAL_MACHINE\SOFTWARE\CPBuildTools\DevContainerHost" /v Path') do set installPath=%%b

  if not defined installPath (
      set "installPath=%programdata%\DevContainerHost"
  )
  ...
  reg add "HKEY_LOCAL_MACHINE\SOFTWARE\CPBuildTools\DevContainerHost" /v "Path" /d "%installPath%" /f
